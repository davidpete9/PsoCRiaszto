...
void pwn1_clk_Start(void) 
{
    ...
    pwn1_clk_CLKEN |= pwn1_clk_CLKEN_MASK;
	pwn1_clk_CLKSTBY |= pwn1_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: pwn1_clk_Stop
********************************************************************************
*
* Summary:
...
void pwn1_clk_Stop(void) 
{
    ...
    pwn1_clk_CLKEN &= (uint8)(~pwn1_clk_CLKEN_MASK);
	pwn1_clk_CLKSTBY &= (uint8)(~pwn1_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: pwn1_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((pwn1_clk_CLKEN & pwn1_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = pwn1_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = pwn1_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* pwn1_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(pwn1_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(pwn1_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: pwn1_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: pwn1_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = pwn1_clk_GetSourceRegister();
    uint16 oldDivider = pwn1_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = pwn1_clk_CLKEN & pwn1_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((pwn1_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: pwn1_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(pwn1_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: pwn1_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    pwn1_clk_MOD_SRC |= modeBitMask & (uint8)pwn1_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: pwn1_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    pwn1_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(pwn1_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: pwn1_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return pwn1_clk_MOD_SRC & (uint8)(pwn1_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: pwn1_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = pwn1_clk_GetDividerRegister();
    uint8 oldSrc = pwn1_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        pwn1_clk_MOD_SRC |= CYCLK_SSS;
        pwn1_clk_MOD_SRC =
            (pwn1_clk_MOD_SRC & (uint8)(~pwn1_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: pwn1_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return pwn1_clk_MOD_SRC & pwn1_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: pwn1_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    pwn1_clk_PHASE = clkPhase & pwn1_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: pwn1_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return pwn1_clk_PHASE & pwn1_clk_PHASE_MASK;
}
