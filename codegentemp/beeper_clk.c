...
/*******************************************************************************
* Function Name: beeper_clk_Start
********************************************************************************
*
* Summary:
...
void beeper_clk_Start(void) 
{
    ...
    beeper_clk_CLKEN |= beeper_clk_CLKEN_MASK;
	beeper_clk_CLKSTBY |= beeper_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: beeper_clk_Stop
********************************************************************************
*
* Summary:
...
void beeper_clk_Stop(void) 
{
    ...
    beeper_clk_CLKEN &= (uint8)(~beeper_clk_CLKEN_MASK);
	beeper_clk_CLKSTBY &= (uint8)(~beeper_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: beeper_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((beeper_clk_CLKEN & beeper_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = beeper_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = beeper_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* beeper_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(beeper_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(beeper_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: beeper_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: beeper_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = beeper_clk_GetSourceRegister();
    uint16 oldDivider = beeper_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = beeper_clk_CLKEN & beeper_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((beeper_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: beeper_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(beeper_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: beeper_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    beeper_clk_MOD_SRC |= modeBitMask & (uint8)beeper_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: beeper_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    beeper_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(beeper_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: beeper_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return beeper_clk_MOD_SRC & (uint8)(beeper_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: beeper_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = beeper_clk_GetDividerRegister();
    uint8 oldSrc = beeper_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        beeper_clk_MOD_SRC |= CYCLK_SSS;
        beeper_clk_MOD_SRC =
            (beeper_clk_MOD_SRC & (uint8)(~beeper_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: beeper_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return beeper_clk_MOD_SRC & beeper_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: beeper_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    beeper_clk_PHASE = clkPhase & beeper_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: beeper_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return beeper_clk_PHASE & beeper_clk_PHASE_MASK;
}
